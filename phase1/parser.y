...
lvalue:		ID {fprintf(stderr, "lvalue -> ID\n");}
			|LOCAL ID {fprintf(stderr, "lvalue -> LOCAL ID\n");}
			|DOUBLE_COLON DOUBLE_COLON ID {fprintf(stderr, "lvalue -> ::ID\n");}
...
block:		LEFT_BRACES stmts RIGHT_BRACES {fprintf(stderr, "block -> [stmts] \n");}
			;

funcdef:	FUNCTION ID LEFT_PARENTHESIS idlist RIGHT_PARENTHESIS block {fprintf(stderr, "funcdef -> function id (idlist) block \n");}
			|FUNCTION LEFT_PARENTHESIS idlist RIGHT_PARENTHESIS block {fprintf(stderr, "block -> function (idlist) block  \n");}
			;
...
idlist:		ID ids {fprintf(stderr, "idlist -> ID ids\n");}
			;

ids:		COMMA ID ids {fprintf(stderr, "ids -> COMMA ID ids\n");}
